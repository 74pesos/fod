...
begin
	...
	while(not EOF(arch)) do begin
		...
		writeln(archTxt,'Empleado: ',act.nombre, ' ',act.apellido,' numero de ID: ',act.nroEmp,' edad: ' ,act.edad,' numero de DNI: ' ,act.dni);
		//writeln(archTxt,cadena); 
	end;
	close(archTxt);
	close(arch);
end;
procedure eliminar(var archivo:tipoArchivo;pos:integer);
var
	rAct:registro;
begin
	reset(archivo);
		...
		case opcion of
			...
			'f': 
				begin
					...
					assign(arch,nombre);
					write('Ingresar posicion a buscar ');readln(pos);
					eliminar(arch,pos);
				end;
		end;
	until(opcion='z');
